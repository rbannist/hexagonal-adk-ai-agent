...
class ApproveMarketingImageCoreService:
    def __init__(
        self,
    ...
    def approve_marketing_image(self, command: ApproveMarketingImageCommand) -> MarketingImage:
        command_data = command.data

        requestor = command_data["requestor"]
        request_id = command_data["request_id"]
        image_id = command_data["image_id"]
        print(f"Handling 'approve marketing image' command for Image: {image_id} with Request ID: {request_id}")
        ...
        marketing_image_approved_most_recent_domain_event = marketing_image.events_list[-1]

        self.aggregate_repository.save(marketing_image)
        print(f"Successfully approved and saved marketing image with ID: {marketing_image.id}")

        # Dispatch the most recent domain event using the dispatcher
        self.domain_event_dispatcher.dispatch(domain_event=marketing_image_approved_most_recent_domain_event)
